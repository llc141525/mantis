...
<template>
  ...
  <v-dialog v-model="dialogVisible" max-width="400">
    <v-card>
      <v-card-item><h3 class="text-h5">Edit Review</h3></v-card-item>
      <v-divider></v-divider>
      <v-card-text>
        <v-label class="mb-2">Product name</v-label>
        <v-text-field single-line variant="outlined" placeholder="Enter Product" model-value="Apple Watch Series 4"></v-text-field>
        <v-label class="mb-2">Author</v-label>
        <v-text-field single-line variant="outlined" placeholder="Enter Author" model-value="Joseph William"></v-text-field>
        <v-label class="mb-2">Review</v-label>
        <v-textarea single-line variant="outlined" placeholder="Text of Review" v-model="text"> </v-textarea>
        <div>Rating</div>
        <v-rating
